...
$sql = "SELECT nombre_integrante, promovidos, fecha FROM contables ORDER BY nombre_integrante ASC";
$query = mysqli_query($koneksidb, $sql) or die ("error : ".mysqli_error($koneksidb));
while($row = mysqli_fetch_array($query)){
	$fecha = $row['fecha'];
	echo "<tr>";
	echo "<td><form method='POST' action='?open=link_cord'><input type='text' name='nombre' value='".$row['nombre_integrante']."' class='hidden' value=''><input type='submit' class='btn btn-link' style='padding : 0px;' value='".$row['nombre_integrante']."'></form></td>";
...
$subsql = "SELECT SUM(promovidos) FROM contables";
